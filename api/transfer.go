...
func (s *Server) createTransfer(ctx *gin.Context) {
	...
	fromAccount, validFromAccount := s.validAccount(ctx, input.FromAccountID, input.Currency)
	if !validFromAccount {
		return
	}
	...
	_, validToAccount := s.validAccount(ctx, input.ToAccountID, input.Currency)
	if !validToAccount {
		return
	}
